...
cfg.tf_data = squeeze(GA_struct.power_co(:,2,3:40,:));
...
cfg.time_i = timeLim;
cfg.freq = GA_struct.freq(3:40);
cfg.tf_data = squeeze(GA_struct.power_ip(:,2,3:40,:));
cfg.zli = [0 0.000001] * 1.05;
%cfg.cluster_mask = false;
cfg.npermutations =10
stat = eegFuture_tfPlot(cfg); hold on
...
cfg.time_i = timeLim;
cfg.freq = GA_struct.freq(3:40);
cfg.tf_data = squeeze(GA_struct.power_co_norm_pre_log(:,2,3:40,:));
cfg.zli = zli;
...
cfg.time_i = timeLim;
cfg.freq = GA_struct.freq(3:40);
cfg.tf_data = squeeze(GA_struct.power_ip_norm_pre_log(:,2,3:40,:));
cfg.zli = zli;
cfg.cluster_mask = true;
cfg.npermutations =10000;
stat = eegFuture_tfPlot(cfg); hold on
...
%
yl = [-0.3 0.6];
data2plot_co= squeeze(nanmean(GA_struct.power_co_norm_pre_log(:,2,freq_i,:),3));
data2plot_ip= squeeze(nanmean(GA_struct.power_ip_norm_pre_log(:,2,freq_i,:),3));
subplot(3,2,5) 
...
saveas(gcf, ['/Users/baiweil/Library/Mobile Documents/com~apple~CloudDocs/Projects/Current Projects/ReAnalysis/EEG_eye_delay_NN/figs_ms/fig3_coVSip.eps'], 'epsc');