...
%%
load('/Users/baiweil/AnalysisDesk/Spontaneous_alpha/EEG_eye_delay_NN/data2plot/GA_saccSize_itpc_topo.mat')
...
for directInd = 1:3
    ...
    dummy.dimord = 'chan_freq_time';
        ...
        dummy.data2plot1 = squeeze(mean(GA_struct.phaseLock_left(:,2,:,:,:))); % mean over trials (reduce to 3 dimensions: electrodes, frequency points, time points (on which we centred our sliding time windows)

        dummy.data2plot2 = squeeze(mean(GA_struct.phaseLock_right(:,2,:,:,:)));
        
        dummy.data2plot3 = squeeze(mean(GA_struct.phaseLock_left(:,2,:,:,:))) -squeeze(mean(GA_struct.phaseLock_right(:,2,:,:,:)));
    % now lets plot it
...
%% ver 2
%% figure 4 plot 
load('/Users/baiweil/AnalysisDesk/Spontaneous_alpha/EEG_eye_delay_NN/data2plot/GA_saccSize_cvsi_TF-PO78.mat')
GA_struct_PO78= GA_struct;
...
timeLim = [-0.5 0.5] ; 
zl = [0 0.25];
figure('position', [100 100 1500 500], 'color', [1 1 1])
...
saveas(gcf, ['/Users/baiweil/Library/Mobile Documents/com~apple~CloudDocs/Projects/Current Projects/ReAnalysis/EEG_eye_delay_NN/figs_ms/fig4_itpc_coVSip.eps'], 'epsc');